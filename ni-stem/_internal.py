...
class StemCreator:
    ...
    def __init__(
        ...
        if numStems > numMetaEntries:
            print(
                "missing stem metadata for stems "
                + str(numMetaEntries)
                + " - "
                + str(numStems)
            )
            ...
            self._metadata.extend(
                self._defaultMetadata["stems"][
                    numMetaEntries : min(numStems, numDefaultEntries)
                ]
            )
            self._metadata["stems"].extend(
                [
                    {
                        "name": "".join(["Stem_", str(i + numDefaultEntries)]),
                        "color": "#000000",
                    }
    ...
    def _convertToFormat(self, trackPath, format):
        ...
        if fileExtension in _supported_files_conversion:
            ...
            if self._format == "aac":
                # AAC
                if _windows and (
                    _findCmd("qaac64") is not None
                    or _findCmd("qaac32") is not None
                    or _findCmd("qaac") is not None
                ):
                    # Use QAAC on Windows if installed
                    qaac = (
                        _findCmd("qaac64")
                        if not None
                        else _findCmd("qaac32") if not None else _findCmd("qaac")
                    )
                ...
                # ALAC
                converterArgs.extend(["-i", trackPath])
                converterArgs.extend(["-c:a", "alac"])  # "alac_at"
                converterArgs.extend(["-c:v", "copy"])
            ...
            print(
                "valid input file formats are " + ", ".join(_supported_files_conversion)
            )
            sys.exit()
    ...
    def save(self, outputFilePath=None):
        ...
        callArgs.extend(
            [
                "-add",
                self._convertToFormat(self._mixdownTrack, format) + "#ID=Z",
                outputFilePath,
            ]
        )
        print("\n[Done 1/6]\n")
        ...
        for stemTrack in self._stemTracks:
            callArgs.extend(
                ["-add", self._convertToFormat(stemTrack, format) + "#ID=Z:disable"]
            )
            conversionCounter += 1
        ...
        if "remixer" in self._tags:
            tags["----:com.apple.iTunes:REMIXER"] = mutagen.mp4.MP4FreeForm(
                self._tags["remixer"].encode("utf-8")
            )
        # mix
        if "mix" in self._tags:
            tags["----:com.apple.iTunes:MIXER"] = mutagen.mp4.MP4FreeForm(
                self._tags["mix"].encode("utf-8")
            )
        # producer
        if "producer" in self._tags:
            tags["----:com.apple.iTunes:PRODUCER"] = mutagen.mp4.MP4FreeForm(
                self._tags["producer"].encode("utf-8")
            )
        # label
        if "organization" in self._tags:
            tags["----:com.apple.iTunes:LABEL"] = mutagen.mp4.MP4FreeForm(
                self._tags["organization"].encode("utf-8")
            )
        if "publisher" in self._tags:
            tags["----:com.apple.iTunes:LABEL"] = mutagen.mp4.MP4FreeForm(
                self._tags["publisher"].encode("utf-8")
            )
        if "label" in self._tags:
            tags["----:com.apple.iTunes:LABEL"] = mutagen.mp4.MP4FreeForm(
                self._tags["label"].encode("utf-8")
            )
        # genre
        ...
        if "track_no" in self._tags:
            if "track_count" in self._tags:
                tags["trkn"] = [
                    (int(self._tags["track_no"]), int(self._tags["track_count"]))
                ]
        # catalog number
        if "catalog_no" in self._tags:
            tags["----:com.apple.iTunes:CATALOGNUMBER"] = mutagen.mp4.MP4FreeForm(
                self._tags["catalog_no"].encode("utf-8")
            )
        # date
        ...
        if "isrc" in self._tags:
            tags["----:com.apple.iTunes:ISRC"] = mutagen.mp4.MP4FreeForm(
                self._tags["isrc"].encode("utf-8"), mutagen.mp4.AtomDataType.ISRC
            )
        # barcode
        if "upc" in self._tags:
            tags["----:com.apple.iTunes:BARCODE"] = mutagen.mp4.MP4FreeForm(
                str(self._tags["upc"]).encode("utf-8"), mutagen.mp4.AtomDataType.UPC
            )
        # cover
        ...
        if "initialkey" in self._tags:
            tags["----:com.apple.iTunes:initialkey"] = mutagen.mp4.MP4FreeForm(
                self._tags["initialkey"].encode("utf-8")
            )
        # key
        if "key" in self._tags:
            tags["----:com.apple.iTunes:KEY"] = mutagen.mp4.MP4FreeForm(
                self._tags["key"].encode("utf-8")
            )
        # album
        ...
        if "mood" in self._tags:
            tags["----:com.apple.iTunes:MOOD"] = mutagen.mp4.MP4FreeForm(
                self._tags["mood"].encode("utf-8")
            )
        # grouping
        ...
        if "barcode" in self._tags:
            tags["----:com.apple.iTunes:BARCODE"] = mutagen.mp4.MP4FreeForm(
                str(self._tags["barcode"]).encode("utf-8"), mutagen.mp4.AtomDataType.UPC
            )
        # lyrics
        ...
        if "url_discogs_artist_site" in self._tags:
            tags["----:com.apple.iTunes:URL_DISCOGS_ARTIST_SITE"] = (
                mutagen.mp4.MP4FreeForm(
                    self._tags["url_discogs_artist_site"].encode("utf-8")
                )
            )
        # url_discogs_release_site
        if "www" in self._tags:
            tags["----:com.apple.iTunes:URL_DISCOGS_RELEASE_SITE"] = (
                mutagen.mp4.MP4FreeForm(self._tags["www"].encode("utf-8"))
            )
        if "url_discogs_release_site" in self._tags:
            tags["----:com.apple.iTunes:URL_DISCOGS_RELEASE_SITE"] = (
                mutagen.mp4.MP4FreeForm(
                    self._tags["url_discogs_release_site"].encode("utf-8")
                )
            )
        # youtube_id
        if "youtube_id" in self._tags:
            tags["----:com.apple.iTunes:YouTube Id"] = mutagen.mp4.MP4FreeForm(
                self._tags["youtube_id"].encode("utf-8")
            )
        # beatport_id
        if "beatport_id" in self._tags:
            tags["----:com.apple.iTunes:Beatport Id"] = mutagen.mp4.MP4FreeForm(
                self._tags["beatport_id"].encode("utf-8")
            )
        # qobuz_id
        if "qobuz_id" in self._tags:
            tags["----:com.apple.iTunes:Qobuz Id"] = mutagen.mp4.MP4FreeForm(
                self._tags["qobuz_id"].encode("utf-8")
            )
        # discogs_id
        if "discogs_release_id" in self._tags:
            tags["----:com.apple.iTunes:Discogs Id"] = mutagen.mp4.MP4FreeForm(
                self._tags["discogs_release_id"].encode("utf-8")
            )
        # media
        if "media" in self._tags:
            tags["----:com.apple.iTunes:MEDIA"] = mutagen.mp4.MP4FreeForm(
                self._tags["media"].encode("utf-8")
            )
        # country
        if "country" in self._tags:
            tags["----:com.apple.iTunes:COUNTRY"] = mutagen.mp4.MP4FreeForm(
                self._tags["country"].encode("utf-8")
            )
